  ...
  export default {
    ...
    methods: {
      ...
      async submitAddProductForm(category) {
        ...
        const response = await fetch(`http://127.0.0.1:5000/user/${user_id}/category/${category.id}/product_api`, {
          method: "POST",
          headers: {
      ...
      async submitEditProductForm(category, product){
        ...
        const user_id = parseInt(localStorage.getItem('userId')); 
        const response = await fetch(`http://127.0.0.1:5000/user/${user_id}/category/${category.id}/product_api/${product.id}`, {
          method: "PUT",
          headers: {
            Authorization: 'Bearer ' + localStorage.getItem('access_token'),
            "Content-Type": "application/json"
